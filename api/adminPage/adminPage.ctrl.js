...
exports.userData = async (req, res) => {
	...
	try {
		...
		if (!userData.length) {
			...
			return res.status(204).json({ status: 204, message: '가입된 유저가 없습니다.' });
		}
...
exports.allowUser = async (req, res) => {
	...
	if (!allowUser) {
		...
		return res.status(404).json({ status: 404, message: '가입 신청이 존재하지 않습니다' })
	}

	try {
		await models.User.allowUser(allow_id);
		await email.sendEmail(allowUser.user_email, '[단디] 회원가입이 승인되었습니다.', email.allowForm());
		return res.status(200).json({ status: 200, message: '회원가입 승인에 성공하였습니다.' });
	} catch (err) {
...
exports.rejectUser = async (req, res) => {
	...
	if (!rejectUser) {
		colorConsole.yellow('[adminPage] 가입신청이 존재하지 않습니다.');
		return res.status(404).json({ status: 404, message: '가입신청이 존재하지 않습니다.' });
	}
	...
	try {
		...
		if (err.message === 'No recipients defined') {
			...
			return res.status(200).json({ status: 200, message: '회원가입 거절에 성공하였습니다.' });
		}
		colorConsole.red(err.message);
