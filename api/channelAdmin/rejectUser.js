...
module.exports = async function (req, res) {
    ...
    try {
        if (!await isFounder(user.user_id, req_channel_id)) { //개설자인지 확인
            console.log("채널유저 거절에 대한 권한이 없습니다 id : " + user.user_id);
            return res.status(403).json({status : 403, message : "채널유저 거절에 대한 권한이 없습니다"});
        }
        ...
        await ChannelUser.destroy({where : {user_id : req_user_id, channel_id : req_channel_id }}); //거절

        console.log("채널에 거절되었습니다 id : " + req_user_id);
        return res.status(200).json({status : 200, message : "채널에 거절되었습니다"});
    } catch(err) {
        console.log("채널 거절중 오류가 발생하였습니다\n" + err);
        return res.status(500).json({status : 500, message : "채널 거절중 오류가 발생하였습니다"});
    }
}