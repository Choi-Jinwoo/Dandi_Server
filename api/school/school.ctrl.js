...
exports.searchByName = async (req, res) => {
	...
	const url = `http://open.neis.go.kr/hub/schoolInfo?SCHUL_NM=${encodeURI(school_name)}&Type=json&KEY=${NEIS_KEY}&pSize=800`;
	let schoolInfo;
...
exports.searchById = async (school_id) => {
	const url = `http://open.neis.go.kr/hub/schoolInfo?SD_SCHUL_CODE=${encodeURI(school_id)}&Type=json&KEY=${NEIS_KEY}`;
		...
		try {
			...
			if (schoolEvent.RESULT !== undefined) {
				if (schoolEvent.RESULT.CODE === 'INFO-200') { //no class info
					...
					return res.status(204).json({ status: 204, message: '학사일정이 존재하지 않습니다.' });
				}
			}
...
exports.classInfo = async (req, res) => {
	...
	const url = `http://open.neis.go.kr/hub/classInfo?SD_SCHUL_CODE=${school_id}&ATPT_OFCDC_SC_CODE=${office_id}&GRADE=${grade}&Type=json&KEY=${NEIS_KEY}`;
