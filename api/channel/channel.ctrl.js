...
exports.joinChannel = async (req, res) => {
	...
	try {
		...
		if (channelUser) {
			...
			return res.status(409).json({ status: 409, message: '이미 가입된 채널입니다.' });
		}
...
exports.leaveChannel = async (req, res) => {
	...
	try {
		...
		if (channel.create_user === user.user_id) {
			...
			return res.status(403).json({ status: 403, message: '탈퇴가 불가능한 유저입니다.' });
		}
		await models.ChannelUser.leaveChannel(user.user_id, channel_id);
...
exports.searchChannel = async (req, res) => {
	...
	try {
		...
		if (!channels.length) {
			colorConsole.yellow('[channel] 채널 정보가 존재하지 않습니다.');
			return res.status(204).json({ status: 204, message: '채널 정보가 존재하지 않습니다.' });
		}
...
exports.channelInfo = async (req, res) => {
	...
	try {
		...
		if (!channelInfo) {
			colorConsole.yellow('[channel] 채널 정보가 존재하지 않습니다.');
			return res.status(404).json({ status: 404, message: '[channel] 채널 정보가 존재하지 않습니다.' });
		}
...
exports.checkChannel = async (req, res) => {
	...
	try {
		...
		if (!joinedChannel.length) {
			...
			return res.status(204).json({ status: 204, message: '가입 채널이 존재하지 않습니다.' });
		}
