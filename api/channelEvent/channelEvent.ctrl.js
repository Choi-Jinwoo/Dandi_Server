...
exports.getChannelEvent = async (req, res) => {
  ...
  try {
    ...
    if (!channel_id) {
      ...
      for (let i = 0; i < joinedChannel.length; i++) {
        events = events.concat(
          await models.ChannelEvent.getEventByChannel(
            joinedChannel[i].channel_id
          )
        );
      }
    } else {
      if (!(await models.ChannelUser.isMember(user.user_id, channel_id))) {
        ...
        return res
          .status(403)
          .json({ status: 403, message: '일정 조회 권한이 없습니다.' });
      }
    ...
    return res
      .status(500)
      .json({ status: 500, message: '일정 조회에 실패하였습니다.' });
  }
};
...
exports.addEvent = async (req, res) => {
  ...
  try {
    if (!(await models.ChannelUser.isMember(user.user_id, channel_id))) {
      ...
      return res
        .status(403)
        .json({ status: 403, message: '일정 추가 권한이 없습니다.' });
    }

    await models.ChannelEvent.createEvent(body);
    return res
      .status(200)
      .json({ status: 200, message: '일정 추가에 성공하였습니다.' });
  } catch (err) {
    colorConsole.red(err.message);
    return res
      .status(500)
      .json({ status: 500, message: '일정 추가에 실패하였습니다.' });
  }
};
...
exports.deleteEvent = async (req, res) => {
  ...
  try {
    ...
    if (!channel_id) {
      ...
      return res
        .status(500)
        .json({ status: 500, message: '일정 삭제에 실패하였습니다.' });
    }
    ...
    if (!(await models.ChannelUser.isMember(user.user_id, channel_id))) {
      ...
      return res
        .status(403)
        .json({ status: 403, message: '일정 삭제 권한이 없습니다.' });
    }
    ...
    return res
      .status(200)
      .json({ status: 200, message: '일정 삭제에 성공하였습니다.' });
  } catch (err) {
    colorConsole.red(err.message);
    return res
      .status(500)
...
exports.searchEvent = async (req, res) => {
  ...
  try {
    if (!(await models.ChannelUser.isMember(user.user_id, channel_id))) {
      ...
      return res
        .status(403)
        .json({ status: 403, message: '일정 검색 권한이 없습니다.' });
    }
    ...
    if (!events.length) {
      ...
      return res
        .status(400)
        .json({ status: 400, message: '검색 결과가 존재하지 않습니다.' });
    }
    ...
    return res
      .status(500)
      .json({ status: 500, message: '일정 검색에 실패하였습니다.' });
  }
};
...
exports.updateEvent = async (req, res) => {
  ...
  try {
    ...
    const event = await models.ChannelEvent.getEvent(event_id);
    body.origin_start_date = event.start_date;
    body.origin_end_date = event.end_date;
  ...
  try {
    ...
    if (!(await models.ChannelUser.isMember(user.user_id, channel_id))) {
      ...
      return res
        .status(403)
        .json({ status: 403, message: '일정 변경 권한이 없습니다.' });
    }

    await models.ChannelEvent.updateEvent(event_id, body);
    return res
      .status(200)
      .json({ status: 200, message: '일정 변경에 성공하였습니다.' });
  } catch (err) {
    colorConsole.red(err.message);
    return res
      .status(500)
      .json({ status: 500, message: '일정 변경에 실패하였습니다.' });
  }
};
